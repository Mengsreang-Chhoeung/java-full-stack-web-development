package com.ms.spring_security_jwt.modules.auth.controller.frontend;
...
import com.ms.spring_security_jwt.infrastructure.annotation.auth.RequireCustomerRole;
...
@Tag(name = ApiDocsConstant.FRONTEND_AUTH, description = ApiDocsConstant.FRONTEND_AUTH_DESCRIPTION)
@ApiDocErrorResponse
@RestController
@RequestMapping(value = RestURIConstant.F_AUTH)
public class FrontendAuthController {
    ...
    @Operation(summary = "For customer login a user")
    ...
    @PostMapping("/login")
    public ResponseEntity<BaseBodyResponse> login(@Valid @RequestBody FrontendAuthLoginRequest request) {
        AuthTokenResponse data = this.authService.frontendLogin(request);
    ...
    @RequireCustomerRole
    @ApiBearerAuth
    @Operation(summary = "For customer get own info")
    @ApiResponse(responseCode = "200", description = ResponseMessageConstant.OPERATION_SUCCESSFULLY, content = {
